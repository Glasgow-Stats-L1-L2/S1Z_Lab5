...
HEARTDISEASE<-read.csv(url("https://raw.githubusercontent.com/Glasgow-Stats-L1-L2/S1Z_Lab5/refs/heads/main/HeartDisease.csv"))
```
...
We will now explore the data before conducting any formal analysis.

For the categorical data of sex, `table()` and `prop.table()` can be handy to tell us the number and percentages of males and females in the data set
...
For when working with a combination of continuous variables like `age` and a binary variable like `TenYearCHD`, box plots can be useful. Currently, our response, `TenYearCHD` is stored as an integer (0 or 1), so we need to specify `as.factor(TenYearCHD)` within the `ggplot()` so that `R` knows to treat it as a binary categorical variable.
